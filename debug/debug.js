...
document
  ...
  .addEventListener("change", async (event) => {
    ...
    try {
      ...
      console.log("Memory after loading program:", cpu.mmu.cartridge);
    } catch (error) {
      console.error(error);
    }

    const title = cpu.mmu.cartridge.subarray(0x0134, 0x0143);
    console.log("Title: " + new TextDecoder().decode(title));

    const manufacturer = cpu.mmu.cartridge.subarray(0x013f, 0x0142);
    console.log("Manufacturer: " + new TextDecoder().decode(manufacturer));

    const licenseCode = cpu.mmu.cartridge.subarray(0x0144, 0x0145);
    console.log("License code: " + licenseCode);

    const sgbFlag = cpu.mmu.cartridge[0x0146];
    console.log("SGB flag: " + sgbFlag.toString(16));

    const cartridgeType = cpu.mmu.cartridge[0x0147];
    console.log("Cartridge type code: " + cartridgeType.toString(16));

    const romSize = cpu.mmu.cartridge[0x0148];
    console.log(
      "ROM size code: " +
        romSize.toString(16) +
        "; Size: " +
