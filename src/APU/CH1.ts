...
export class CH1 extends CH2 {
  ...
  sweepTimerCycles: number = 32768; // Remaining cycles to tick up sweepTimer (32768 cycles, 128 Hz)
  sweepTimer: number = 0; // Increments when sweepTimerCylces reaches 0
  ...
  public update(cycles: number) {
    ...
    if (this.sweepTimerCycles <= 0) {
      ...
      if (
        ...
        if (this.source && this.apu.audioCtx) {
  ...
  private calculateSweepFreq(direction: boolean) {
    const currentPeriod = this.currentPeriod;
    const periodDivider = currentPeriod / Math.pow(2, this.getSweepStep());
    const newPeriod = currentPeriod + (!direction ? periodDivider : -periodDivider);

    return newPeriod;
  }

  private updatePeriod(newPeriod: number) {
