...
// TODO: replace `any` with CPU interface when available
export class Disassembler {
  ...
  disassemble(romSize: number): void {
    ...
    for (let i = 0; i < romSize; i++) {
      ...
      if (instruction.length > 1 && opcode !== 0xcb) {
        ...
        instruction = instruction.map((operand: any) => {
          let immediate: any;
          ...
          if (operand === "d16" || operand === "a16") {
            ...
            const low = this.cpu.mmu.cartridge[i + 1]
              .toString(16)
              .padStart(2, "0");
            const high = this.cpu.mmu.cartridge[i + 2]
              .toString(16)
              .padStart(2, "0");
