...
type FrameBufferLike = ImageData | { data: Uint8ClampedArray; width: number; height: number };

export class PPU {
  // Public IO register addresses (kept public for debug/test access)
  public lcdc = 0xff40; // LCD Control
  public stat = 0xff41; // LCD status
  public scy = 0xff42; // Background viewport Y position
  public scx = 0xff43; // Background viewport X position
  public ly = 0xff44; // LCD Y coordinate [Read-only]
  public lyc = 0xff45; // LY compare
  public dma = 0xff46; // OAM DMA source address & start
  public bgp = 0xff47; // BG palette data
  public obp0 = 0xff48;
  public obp1 = 0xff49;
  public wy = 0xff4a;
  public wx = 0xff4b;
  ...
  public cpu: any;
  public mmu: any;
  public screen: HTMLCanvasElement | null = null;
  public context: CanvasRenderingContext2D | null = null;
  public frameBuffer: FrameBufferLike;

  public oam: Uint8Array; // OAM   160 B.   Area FE00 - FE9F
  public vram: Uint8Array; // VRAM   8 KiB.   Area 8000 - 9FFF

  public palettes: any;
  public selectedPalette: any;

  private line: number;
  private windowLine: number;
  ...
  constructor(cpu: any) {
    ...
    // Headless-safe initialization: if document is undefined, avoid touching DOM
    const HEADLESS = typeof document === "undefined" || !document.getElementById;
    ...
    if (HEADLESS) {
      this.screen = null;
      this.context = null;
      this.frameBuffer = { data: new Uint8ClampedArray(DEFAULT_WIDTH * DEFAULT_HEIGHT * 4), width: DEFAULT_WIDTH, height: DEFAULT_HEIGHT };
    } else {
      this.screen = document.getElementById("screen") as HTMLCanvasElement | null;
      if (this.screen) {
        this.context = this.screen.getContext("2d", { willReadFrequently: true });
        const w = this.screen.width || DEFAULT_WIDTH;
        const h = this.screen.height || DEFAULT_HEIGHT;

        if (this.context && typeof (this.context as any).createImageData === "function") {
          // createImageData may throw in some environments; fallback to manual buffer
          try {
            this.frameBuffer = (this.context as CanvasRenderingContext2D).createImageData(w, h);
          } catch (e) {
            this.frameBuffer = { data: new Uint8ClampedArray(w * h * 4), width: w, height: h };
          }
        } else {
          this.context = null;
          this.frameBuffer = { data: new Uint8ClampedArray(w * h * 4), width: w, height: h };
        ...
        // No canvas found, behave headless
        this.screen = null;
        this.context = null;
        this.frameBuffer = { data: new Uint8ClampedArray(DEFAULT_WIDTH * DEFAULT_HEIGHT * 4), width: DEFAULT_WIDTH, height: DEFAULT_HEIGHT };
      }
  ...
  private drawScanLine() {
    ...
    if (renderBGWin || renderSprites) {
      ...
      if (this.context && typeof (this.context as any).putImageData === "function") {
        // If frameBuffer is not a real ImageData, construct one from raw buffer
        if (typeof (window as any) !== "undefined" && (this.frameBuffer as ImageData).data) {
          this.context.putImageData(this.frameBuffer as ImageData, 0, 0);
        } else {
          // Create ImageData on-the-fly in browser env
          try {
            const img = new ImageData((this.frameBuffer as any).data, (this.frameBuffer as any).width, (this.frameBuffer as any).height);
            this.context.putImageData(img, 0, 0);
          } catch (e) {
            // ignore in headless or unsupported envs
          }
        }
      }
    }
  ...
  private renderBGWin() {
    ...
    const imageData = (this.frameBuffer as any).data as Uint8ClampedArray;
    ...
    for (let pixel = 0; pixel < 160; pixel++) {
      ...
      const pixelIndex = (ly * (this.frameBuffer as any).width + pixel) * 4;
  ...
  private renderSprites() {
    ...
    for (let sprite = 0; sprite < 40; sprite++) {
      ...
      for (let tilePixel = 7; tilePixel >= 0; tilePixel--) {
        ...
        const { r: red, g: green, b: blue } = this.getColour(colourNum, colourAddress);
        ...
        const pixelIndex = (ly * (this.frameBuffer as any).width + pixel) * 4;
        const isBgWhite = imageData[pixelIndex] == 255 || imageData[pixelIndex] == 155;
  ...
  private getColour(colourNum: number, paletteAddr: number) {
  ...
  private setLY(val: number) {
    this.mmu.ioRegs[this.ly & 0x7f] = val;
  }

  private doDMATransfer(data: number) {
    const address = data << 8; // source address is data * 100
    for (let i = 0; i < 160; i++) this.mmu.writeByte(0xfe00 + i, this.mmu.readByte(address + i));
  }
